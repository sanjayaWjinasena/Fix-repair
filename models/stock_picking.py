...
class StockPicking(models.Model):
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type == 'form':
            ...
            # Lock repair-flow picking fields ALWAYS, from creation
            # onward. Covers both flavours:
            #
            #   * SO-driven DELIVERY pickings (quotation_type='Repair')
            #     — auto-generated by Odoo stock rules from repair
            #     sale.order lines. Moves + quantities are populated
            #     by the SO; the operator's only legitimate action is
            #     to click Validate.
            #   * MOVEMENT pickings (helpdesk_ticket_id stamped by
            #     _create_returns and the Send-to-Factory /
            #     Received-at-Factory / Send-to-Sales-Centre /
            #     Dispatch server actions) — same story, every field
            #     is pre-populated by our own code.
            #
            # v217 froze movement pickings pre-validate but still let
            # delivery pickings edit until state='done'. v219 aligns
            # both: any repair-related picking is validate-only. This
            # matches the workflow contract — editing partner_id /
            # origin / picking_type_id on either type would desync the
            # ticket's smart buttons and downstream stage transitions.
            #
            ...
            # them clickable.
            #
            # not(ancestor::field): skip fields inside embedded views
            # (move_ids_without_package, move_line_ids etc.). Those
            # sub-rows are stock.move / stock.move.line — they don't
            # have x_studio_quotation_type or x_studio_helpdesk_ticket_id,
            # so stamping the picking-level lock expression on them
            # would raise "Name not defined" during list-cell
            # readonly eval. The qty_done / lot_id cells inside
            # move_line_ids stay editable so a mispopulated serial
            # can still be corrected pre-validate.
            lock_gate = (
                "x_studio_quotation_type == 'Repair' "
                "or x_studio_helpdesk_ticket_id"
            )
